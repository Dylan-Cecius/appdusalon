-- Réecrire la fonction avec une logique de fuseau horaire plus simple et correcte
...
BEGIN
  ...
  -- Obtenir l'heure actuelle à Paris (sans timezone)
  paris_time := NOW() AT TIME ZONE 'Europe/Paris';
  
  CASE frequency_type
    WHEN 'daily' THEN
      -- Pour quotidien, prendre le jour suivant à l'heure spécifiée
      local_next_date := (paris_time::DATE + INTERVAL '1 day')::TIMESTAMP
      ...
      IF MAKE_TIME(target_hour, target_minute, 0) > paris_time::TIME THEN
        local_next_date := paris_time::DATE::TIMESTAMP + MAKE_INTERVAL(hours => target_hour, mins => target_minute);
      END IF;
    ...
    WHEN 'weekly' THEN
      ...
      BEGIN
        ...
        local_next_date := (paris_time::DATE + INTERVAL '1 day' * days_until_target)::TIMESTAMP
                          + MAKE_INTERVAL(hours => target_hour, mins => target_minute);
      END;
    ...
    WHEN 'monthly' THEN
      ...
      BEGIN
        ...
        local_next_date := target_date::TIMESTAMP + MAKE_INTERVAL(hours => target_hour, mins => target_minute);
      END;
  ...
  -- Traiter la date locale comme étant en heure de Paris et la convertir en UTC
  next_date := local_next_date AT TIME ZONE 'Europe/Paris';
...
-- Remettre à jour tous les rapports existants
