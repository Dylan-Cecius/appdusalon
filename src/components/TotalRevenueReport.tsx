...
import { CalendarDays, CalendarRange, CalendarCheck, CalendarIcon, Search } from 'lucide-react';
...
const TotalRevenueReport = () => {
  ...
  const caJour = useMemo(() => {
    ...
    return transactions.filter(tx => { const d = new Date(tx.transactionDate); return d >= s && d <= e; }).reduce((sum, tx) => sum + tx.totalAmount, 0);
  }, [transactions]);
  ...
  const txCount = useMemo(() => {
    ...
    const s = startDate ? startOfDay(startDate) : new Date(0);
    const e = endDate ? endOfDay(endDate) : new Date();
    return transactions.filter(tx => { const d = new Date(tx.transactionDate); return d >= s && d <= e; }).length;
  }, [transactions, startDate, endDate]);

  const stats = [
    { label: "CA du jour", value: caJour, icon: CalendarDays, gradient: 'from-emerald-500 to-emerald-600', glow: 'shadow-emerald-500/20' },
    { label: "CA de la semaine", value: caSemaine, icon: CalendarRange, gradient: 'from-blue-500 to-blue-600', glow: 'shadow-blue-500/20' },
    { label: "CA du mois", value: caMois, icon: CalendarCheck, gradient: 'from-violet-500 to-violet-600', glow: 'shadow-violet-500/20' },
  ];
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("w-[170px] justify-start text-left font-normal", !startDate && "text-muted-foreground")}>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className={cn("w-[170px] justify-start text-left font-normal", !endDate && "text-muted-foreground")}>
                <CalendarIcon className="mr-2 h-4 w-4" />
        ...
        {caCustom !== null && (
          <div className="mt-5">
            <Card className="relative overflow-hidden p-6 sm:p-8 flex flex-col items-center justify-center text-center shadow-xl shadow-amber-500/20">
              ...
              <div className="inline-flex items-center justify-center w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 text-white mb-4 shadow-lg shadow-amber-500/20">
                <CalendarRange className="h-7 w-7" />
              </div>
              <p className="text-3xl sm:text-4xl font-extrabold tracking-tight text-foreground">
                {caCustom.toFixed(2)}€
              </p>
              <p className="text-sm text-muted-foreground mt-2 font-medium">{customLabel}</p>
              <p className="text-xs text-muted-foreground mt-1">{txCount} transaction{txCount > 1 ? 's' : ''}</p>
            </Card>
          </div>
        )}
      </Card>
